...
export default function ModifyPlayList() {
  ...
  const [loadingPage, setLoadingPage] = useState(true);
    const { plid, currname } = useLocalSearchParams<any>(); 
  ...
  async function init() {
    try {
      setLoadingPage(true);
      setNamePlaylists(currname);
      setActiveButton(currname.length > 0)
        ...
        <View className="w-full">
          <View className="flex flex-row justify-between mb-5">
            ...
            <View className="flex flex-row justify-center items-center h-full">
              <Text className="text-xl font-semibold">Đổi tên playlist</Text>
            </View>
          ...
          <View className="w-[90%] max-w-[900px]">
            <Pressable
              onPress={() => {
                if (activeButton) {
                  try {
                    playlistBUS.updatePlaylist(FIREBASE_AUTH.currentUser!.uid as string, plid, namePlaylists);
                    DeviceEventEmitter.emit("changePlaylistName", "success");
                    router.back();
                  } catch (e) {
                    alert(e);
                  }
